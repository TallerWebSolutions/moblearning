...
const AdminPage = props => {
  ...
  const [activeLessonId, setActiveLessonId] = useState(
    "0eea2b6e-dddf-43c8-b7ee-991d03651136"
  )
  const activeLesson = formik.values.lessons.find(
  ...
  const addContent = (type, formik, activeLessonId) => () => {
    ...
    const values = Object.keys(widget.fields).reduce(
      (acc, fieldName) => ({
        ...acc,
  ...
  const setActiveLessonFieldValue = (contentIndex, name, value) => {
    ...
    formik.setFieldValue(
      `lessons[${index}].contents[${contentIndex}].values.${name}`,
      value
    )
  }
                ...
                <div>
                  ...
                  {!showWidgets && (
                    <Box
                      ...
                      justifyContent="center"
                    >
                      <Fab onClick={() => setShowWidgets(true)}>
                        <AddIcon />
                      </Fab>
                    </Box>
                  )}
